...
تا قبل از es6 فقط فانکشن‌ها بودن که scope جدید می‌ساختن  ولی الان مفهوم block scoping هم اضافه شده که با استفاده از const و let می‌تونید این کارو انجام بدید. (البته برای قبل از es6 هم تکنیک‌هایی وجود داشت برای شبیه سازی block scoping ولی خب الان نیازی نیست واردش بشیم.)
...
function hello(){
  ...
  console.log(world()); // Scope 1
}

hello();
```

سه تا فانکشن با نام‌های hello و world و js داریم که هر کدوم یک scope جدید درست کردن و توی فانکشن world اگر دقت کنید٬ می‌بینید که متغیر A داره برگردونده میشه که در واقع با استفاده از رفتار lexical scope جاوااسکریپت ما به متغیر scope بالاتر دسترسی داریم.
...
console.log(myName()); // mojtabast
console.log(myName2()); // mojtaba
```

توی es6 به جای var میتونیم از let و const استفاده کنیم که در در واقع block scope هستن. block scope هم با `{}` تعریف می‌شه.
...
یک مبحث دیگه‌ای هم که توی Scope مطرحه **Hoisting** هستش. Hoisting یک مفهومی توی جاوااسکریپت هستش که توی زمان کامپایل اتفاق میفته و به اینصورته که تعریف متغیر و فانکشن به بالای Scope‌ـی که توش هست منتقل میشه خود به خود.

کد زیر رو در نظر بگیرید و حدس بزنید که خروجی چیه:
...
در واقع جاوااسکریپت کد شما رو به همچین چیزی تبدیل می‌کنه:
...
برای اینکه عمیق‌تر بشید روش کلی مقاله‌ی خوب انگلیسی هستش که راحت می‌تونید پیدا کنید و اگر فرصت بشه در یک پست جداگونه منم بهش می‌پردازم چون هم خیلی استفاده میشه و هم اینکه خیلی مهمه به نظرم که درک بشه کامل.
 ...
 یک تاپیک دیگه هم که هست توی موضوع **Higher Order Function** هستش. فکر می‌کنم که خیلی پیچیده نیست و با یه جستجوی ساده توی گوگل به راحتی میشه فهمید چیه.
...
- Block Scope, const & let: در مورد بلاک اسکوپ بالاتر توضیح دادم و علاوه بر `var` می‌تونید از `let` و `const` برای تعریف متغیر استفاده کنید که هم بلاک‌اسکوپ هستن و همین اینکه `const` شما نمی‌تونید reassignment (مفهومش به فارسی میشه «دوباره مقداردهی کردن» اگه اشتباه نکنم) کنید.
- Class: این ویژگی بیشتر به نظرم Sugar Syntax هستش و ممکنه یکم گیج‌کننده هم باشه چون کلاس توی جاوااسکریپت نحوه‌ی عمکلکردش با زبون‌های برنامه‌نویسی دیگه فرق میکنه. پس صرفن اینو در نظر داشته باشید که کلس‌ها توی جاوااسکریپت دقیقن همون Function Constructorهایی هستن که شما توی ES5 می‌نوشتید.
...
- Arrow functions: باهاش میشه فانکشن درست کرد ولی تفاوتش با بقیه اینکه که `this` به اسکوپ بالاسریش اشاره می‌کنه و دیگه اسکوپ جدید ساخته نمیشه که به اسکوپ خود فانکشن اشاره کنه. این مورد رو خیلی توی اضافه کرد event listener به یک المنتی توی صفحه احتمالن دیدید و مجبور بودید `this` رو توی یک متغیر دیگه (معمولن `that` میذاشتن اکثرن) بذاریمش. یا اینکه از `bind` استفاده می‌کردید.
...
- Promise & async await: جاوااسکریپت موقع اجرا برای چیزی صبر نمی‌کنه و Asynchronize هستش که خیلی سادش این میشه که این کارو انجام بده هر وقت خبری شد بهمون بگو. به عنوان مثال یک ریکوئست می‌زنید به سرور و ۱۰ ثانیه طول می‌کشه٬ طبیعتن کل برنامه نباید متوقف بشه و هر وقت از سرور چیزی گرفتیم متقابلن کاری که می‌خوایم رو باهاش انجام بدیم. شیوه‌ی قدیمیش این بود که یک فانکشن به عنوان کال‌بک ست می‌کردی و هر موقع اون کار انجام میشد٬ تهش این کال‌بک صدا زده میشد. بعدش Promise اضافه شد که خیلی راحت به صورت زنجیره‌وار می‌تونسید متقابلن کاری رو انجام بدید و یک سری ویژگی‌های دیگه. اخیرن هم [async await](https://alligator.io/js/async-functions/) اضافه شده که خیلی تمیزتر و بهتر هستش استفادش به نظرم.
- Destructing: می‌تونید خیلی راحت از یک آبجکت چیزایی که می‌خواید رو بکشید بیرون و بریزید توی متغیری به همون اسم.
...
- Rest & spread: همونطور که می‌بینید سه نقط خیلی کاربردای باحالی پیدا کرده که دو  تا دیگه از کاربرداش هم توی مثال زیر می‌تونید ببینید.
...
var countries = {
  ...
  IR: 'IRAN',
  USA: 'United States',
  ...africa, // Spread Operat
  ...europe // Spread Operat
};

var {IR, ...restOfCountries} = countries; // Resting Operator
...
همونطوری که می‌دونی ماژول‌ها با این هدف درست میشن که کار به قسمت‌های کوچک‌تری شکسته بشه و هر قسمتی به صورت مجزا یک کاری رو انجام بده. اینطوری هم نگهداری و توسعه کار راحت‌تره و هم اینکه چون encapsulate هستن به مشکلات ناخواسته‌ای نمی‌خورید مثل مشکلات اسم‌گذاری.
...
این چه کمکی می‌کنه؟ همونطور که گفتم توی جاواسکریپت فانکشن هستش که اسکوپ جدید درست می‌کنه به همین خاطر اینجا ما در واقع کدارو داخل این فانکشن می‌ذاریم و کدای داخلش encapsulate یا ایزوله میشن و در نهایت این فانکشن به ما یک API یا اینترفیس میده که میتونیم از ماژول instance بگیریم و ازش استفاده کنیم. این تکنیک یک سری نکات پرفورمنسی و شیوه‌ی نوشتن هم داره که می‌تونید با یه سرچ پترن‌های مختلفش رو ببینید و آشنا شید باهاش.

### CommonJS

تا قبل از ES6 جاوااسکریپت خودش مکانیزمی واسه ماژول‌ها نداشت به خاطر همین فکر کنم یکی از مهندسای موزیلا یک مکانیزمی با یک سری استانداردای خودش درست کنه. دو سه سال بعد Node.js اونو پیاده‌سازی کرد برای خودش و توی اون اکوسیستم وارد شد. منتها موقع طراحی این مکانیزم سمت سرور ملاک قرارداده شده (حتی توی نسخه‌ی ابتداییش هم اسمش ServerJS بوده که بعد تغییر نام داده شد به CommonJS) به خاطر همین یک سری مشکلات داره برای مرورگر و سمت کاربر که مهم‌ترینش Asynchronize نبودنش هست. و در آخر این رو هم اضافه بکنم که CommonJS فقط یک مکانیزمی برای ماژول نبوده بلکه یک پروژه‌ای بوده با این هدف که یک سری API‌های متداول رو به جاوااسکریپت اضافه کنه که [ماژول](http://wiki.commonjs.org/wiki/Modules/1.1) هم یکی از بخشاش بوده. نمونه:
